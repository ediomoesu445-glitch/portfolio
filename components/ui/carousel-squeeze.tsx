...
/**
 * Vendored from 21st.dev, with three deliberate changes. Keep them in mind if
 ...
 * 2. `<img>` became `next/image`, so panel art goes through the same optimiser,
 *    lazy loading and intrinsic-dimension manifest as every other image here.
 * 3. Colours address the theme through the shadcn aliases in globals.css,
 *    which map onto the Control Room palette.
 */
...
export function SqueezeCarousel({
  ...
  // Upstream seeded these keys from a ref inside the initialiser, which runs
  // during render. Keys only need to be unique and stable, so the opening set
  // is numbered positionally and the counter starts just past them.
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  "focus-visible:ring-offset-background focus-visible:ring-[var(--sq-fill)]",
                  panelClassName,
                )}
                style={{
                  width: widthOf(col),
 ...
 * manifest, and the optimiser and lazy loading apply as they do everywhere else.
 */
function Picture({ slide }: { slide: SqueezeSlide }) {
  ...
  if (slide.image) {
    const dimensions = getDimensions(slide.image);
    return (
      <Image
        src={slide.image}
        alt={slide.imageAlt ?? ""}
